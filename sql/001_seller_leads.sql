-- Sell My House Today Anywhere - Phase 1 seller leads table
-- Run this in Supabase SQL editor before testing the lead form.
...
create index if not exists seller_leads_property_state_idx on public.seller_leads (property_state);

alter table public.seller_leads enable row level security;

-- The app uses the Supabase service role key from server-only routes.
